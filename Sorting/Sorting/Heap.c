...
void MakeHeap(int array[], int size, int index) { //
	...
	if (max != index) { //만약 같으면 자기자신가 가장 큰 원소를 가지고 있는것이 됨. 
		...
		Swap(&array[index], &array[max]); //큰 원소를 부모로 올리는 작업.
		MakeHeap(array, size, max); //다음 레벨(트리 구조의 level)로 올라감.
	}
}

void HeapSort(int array[]) {
	for (int i = ARRAYSIZE / 2 - 1; i >= 0; i--) //최상단 노드(array[0])을 찾아가면서 최대 힙을 구성 
		MakeHeap(array, ARRAYSIZE, i);

	for (int i = ARRAYSIZE - 1; i >= 0; i--) { 
		//최대 힙에서 최상단의 노드(array[0])가 가장 큰 숫자이므로 
		//현재(반복문 범위 내의) 힙에서 가장 큰 숫자를 i위치와 바꾸면
		//i에 현재 가장 큰 숫자가 위치하게 된다. 그리고 힙을 재구성.
		//이 과정을 i = 0 까지 하면 큰 숫자부터 순서대로 역으로 정렬됨.
		Swap(&array[i], &array[0]); // 즉, 이 과정은 힙에서 노드를 빼는것과 같음
		MakeHeap(array, i, 0); //다시 최대힙을 구성함
	}
}

void makeheap(int array[], int size) { //최대 힙
	for (int i = 1; i < size; i++) {
		int child = i;
		int root = (child - 1) / 2;
		if (array[child] > array[root]) { //자식 노드가 부모 노드보다 작아야 함
...
void heapsort(int array[]){ 
	...
	for (int i = ARRAYSIZE - 1; i >= 0; i--) {
		...
		do {
			...
			if (array[index] < array[child] && child < i) //부모 노드가 자식 노드보다 작으면 교환
...
void Heap(int* ToSort) {
	//HeapSort(ToSort);  //재귀문 사용
	heapsort(ToSort); //반복문 사용
}
